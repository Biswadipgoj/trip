...
export function AppNav({ tripId }: AppNavProps) {
  ...
  const members = useStore(s => s.members.filter(m => m.tripId === tripId))
  const expenses = useStore(s => s.expenses.filter(e => e.tripId === tripId))
  const settlements = useStore(s => s.settlements.filter(st => st.tripId === tripId))
